...
Adam::Adam(const std::vector<std::reference_wrapper<Tensor>> &params, double learning_rate, const AdamOptions &options)
    : learning_rate_(learning_rate), options_(options) {
    add_parameters(params);
}

void Adam::add_parameters(const std::vector<std::reference_wrapper<Tensor>> &params) {
...
void Adam::save(const std::string &path) const {
