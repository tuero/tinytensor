...
template <typename T, typename SizeType, std::size_t Length>
struct IsLogicalBufferPair<const std::array<T, Length>, SizeType> : std::true_type {};

// Enable if BufferType and SizeType constitute a valid logical buffer pair.
template <typename BufferType, typename SizeType>
using EnableIfLogicalBufferPair = typename std::enable_if<IsLogicalBufferPair<BufferType, SizeType>::value>::type;
...
template <typename BufferType, typename SizeType, bool IsUnbounded = false, typename Enabled = void>
class LogicalBuffer {
...
struct LogicalBufferStaticConstraints {
    ...
    static_assert(ArrayTraits<BufferType>::Length <= std::numeric_limits<SizeType>::max(),
...
class LogicalBuffer<BufferType, SizeType, IsUnbounded_, EnableIfLogicalBufferPair<BufferType, SizeType>>
...
public:
    ...
    enum : bool { IsTriviallyDestructible = std::is_trivially_destructible<ValueType>::value };

    static_assert(!(Length != 1 && IsUnbounded), "Unbounded logical buffers must have an array element of length 1!");
    static_assert(!(IsUnbounded && !IsTriviallyDestructible),
                  "Unbounded logical buffers must have trivially destructible "
                  "value types!");

    constexpr LogicalBuffer(BufferType& data, SizeType& size) : data_{data}, size_{size} {}
    ...
    constexpr const SizeType& size() const {
        return size_;
    }

    constexpr ValueType* begin() {
        return &data_[0];
    }
